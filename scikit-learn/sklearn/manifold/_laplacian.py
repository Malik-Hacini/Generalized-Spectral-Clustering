...
class Laplacian:
    ...
    def __init__(self, adjacency, standard=False, measure=None):
        ...
        degree_vec = (
            self.adjacency.sum(axis=1).A1
            if self.is_sparse
            else self.adjacency.sum(axis=1)
        )
        ...
        if self.standard:
            ...
            if isinstance(self.measure, tuple):
                ...
                if gamma == 1:
                    ...
                    P_gamma_t = np.linalg.matrix_power(
                        (1 - gamma) * np.full((self.N, self.N), 1 / self.N) + gamma * P,
                        t,
                    )
                v = (
                    np.array(
                        ((1 / self.N) * np.ones((1, self.N)) @ P_gamma_t)
                    ).T.flatten()
                ) ** alpha
            else:
