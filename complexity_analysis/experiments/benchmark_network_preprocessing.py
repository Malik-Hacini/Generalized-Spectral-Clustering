"""Benchmark SC and GSC preprocessing on sparse DISBM networks."""
...
def main() -> None:
    ...
    for n_nodes in parse_int_list(args.n_values):
        ...
        def sc_preprocessing() -> None:
            W_sym = symmetrize_graph(adjacency)
            build_standard_normalized_laplacian(W_sym)

        def gsc_preprocessing() -> None:
            P = build_transition_matrix(adjacency)
            nu = compute_measure_from_transition(P, alpha=args.alpha, t=args.t)
            xi = compute_xi(P, nu)
            build_generalized_normalized_laplacian(P, nu, xi)

        for series, func in (("SC-pre", sc_preprocessing), ("GSC-pre", gsc_preprocessing)):
            for repeat in range(args.repeats):
                ...
                raw_rows.append(
                    {
                        "benchmark": "network_preprocessing_disbm",
                        "series": series,
                        "x_name": "m",
                        ...
                        "degree_factor": args.degree_factor,
                        "t": args.t if series == "GSC-pre" else float("nan"),
                        "alpha": args.alpha if series == "GSC-pre" else float("nan"),
    ...
    paths = save_outputs(
        benchmark_name="network_preprocessing_disbm",
        raw_df=raw_df,
