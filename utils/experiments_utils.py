...
def experiment(
    ...
    if mode not in ("score", "viz", "grid_search"):
        raise ValueError(
            f"Unknown experiment mode: {mode}. Must be 'score', 'viz', or 'grid_search'"
        )

    if not isinstance(metrics, (str, tuple, list)):
        raise ValueError(
            "Metrics must be a string (for a single metric) or a tuple/list of strings."
        )
...
def clusterer(method_name, params):
    ...
    if method_name == "spectral":
        ...
        raise ValueError(
            f"Unknown clustering method: {method_name}. Supported methods: 'spectral', 'kmeans', 'hdbscan"
        )


def _run_grid_search_experiment(
    dataset_names, method_specs, config, load_path, metrics, n_jobs
):
    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        for implicit_name, explicit_name in method_specs:
            ...
            grid_params, base_params, param_names, combinations = _resolve_grid_params(
                resolved_params
            )

            if not grid_params:
                logger.info(
                    f"  No grid parameters specified for {explicit_name}, running single combination..."
                )
                combinations = [()]
                param_names = []
            else:
            ...
            try:
                ...
                logger.error(
                    f"Invalid n_it parameter for {explicit_name} on {dataset_name}: {n_it}. Must be a positive integer."
                )
                continue
            ...
            if "error" in method_results:
                logger.error(
                    f"  {explicit_name} on {dataset_name}: {method_results['error']}"
                )
                continue
            ...
            if len(metrics) == 1:
                ...
                if target_metric in method_results:
                    ...
                    logger.info(
                        f"  → {_format_best_metric_log(target_metric, best_score_info):s}"
                    )
            else:
...
def _run_viz_experiment(dataset_names, method_specs, config, load_path, metrics):
    ...
    for i_dataset, dataset_name in enumerate(dataset_names):
        ...
        if X.shape[1] != 2:
            logger.info(
                f"Warning: Dataset {dataset_name} is not 2D (shape: {X.shape}). Skipping visualization."
            )
            plot_num += len(method_specs)
            continue
        ...
        for j_method, method_spec in enumerate(method_specs):
            ...
            params = config.get_final_params(
                y, dataset_name, explicit_name, implicit_name
            )
            ...
            if error:
                logger.error(
                    f"Error processing {explicit_name} on {dataset_name}: {error}"
                )
                plt.text(
                    0.5,
                    0.5,
...
def _run_single_task(task, mode):
    ...
    for i in range(n_it):
        ...
        if iteration_error:
            logger.error(
                f"Error processing {explicit_name} on {dataset_name}: {iteration_error}"
            )
            return None
        ...
        with warnings.catch_warnings():
            ...
            if hasattr(cluster_obj, "fit_predict"):
                ...
                y_pred = (
                    ...
                    else cluster_obj.predict(X)
                )
            else:
                raise ValueError(
                    f"Clustering object for {implicit_name} has neither fit nor fit_predict method"
                )

        y_pred = y_pred.astype(int)

        metric_params = params.get("metric_params", None)
        scores = _compute_clustering_scores(
            y, y_pred, metrics, X, metric_params=metric_params
        )
...
def _compute_clustering_scores(y_true, y_pred, metrics, X, metric_params=None):
    ...
    if metric_params is None:
        ...
        raise ValueError(
            "'metric_params' must be a dict mapping metric names to kwargs dicts"
        )
...
def _create_cluster_plot(
    X, y_pred, explicit_name, dataset_name, timing, i_dataset, j_method, scores, metrics
):
...
def _aggregate_grid_search_results(results, metrics):
    ...
    for metric in metrics:
        ...
        if metric_scores:
            ...
            best_payload = {
                "mean": (
                    best_score_dict["mean"]
                    if isinstance(best_score_dict, dict)
                    else best_score_dict
                ),
                "std": std_across_grid,
            }
            if linked_ami_score is not None:
                best_payload["linked_ami_mean"] = (
                    linked_ami_score["mean"]
                    if isinstance(linked_ami_score, dict)
                    else linked_ami_score
                )
