...
def generate_point_cloud(N, D, K=3, seed=42):
    ...
    X, y = make_blobs(
        n_samples=N, n_features=D, centers=K, cluster_std=2.5, random_state=seed
    )
...
def analyze_and_plot(df_pointcloud, df_network):
    ...
    plt.tight_layout()
    fig1.savefig(
        f"{RESULTS_PATH}/complexity_pointcloud_comparison.pdf", bbox_inches="tight"
    )
    fig1.savefig(
        f"{RESULTS_PATH}/complexity_pointcloud_comparison.png",
        bbox_inches="tight",
        dpi=300,
    )
